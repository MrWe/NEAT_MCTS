...
class ConfigParameter(object):
    ...
    def interpret(self, config_dict):
        ...
        raise RuntimeError(
            "Unexpected configuration type: " + repr(self.value_type))
...
def write_pretty_params(f, config, params):
    ...
    for name in param_names:
        p = params[name]
        f.write('{} = {}\n'.format(p.name.ljust(longest_name),
...
class DefaultClassConfig(object):
    ...
    def __init__(self, param_dict, param_list):
        ...
        unknown_list = [x for x in iterkeys(
            param_dict) if not x in param_list_names]
        if unknown_list:
